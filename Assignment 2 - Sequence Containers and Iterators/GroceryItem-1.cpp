...
namespace    // unnamed, anonymous namespace
{
  ...
  constexpr bool floating_point_is_equal( T const lhs,  U const rhs,  double const EPSILON = 1e-4 ) noexcept
  {
    ///////////////////////// TO-DO (1) //////////////////////////////
      ...
      ///         do this instead:
      ///           return a < b;
      return std::abs(lhs-rhs) < EPSILON;   //compare two floating point numbers, will return true when both numbers are within Epsilon, and
...
: _upcCode     {std::move(upcCode)},
  ...
  _price       {price}                                //we dont move double data types because it is very cheap to make a copy of it
{
}
/////////////////////// END-TO-DO (2) ////////////////////////////
...
///////////////////////// TO-DO (5) //////////////////////////////
{
  if (this != &rhs)   //self assignment guard - to prevent assigning copy constructor on itself
  {
...
std::weak_ordering GroceryItem::operator<=>( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (19) //////////////////////////////
  auto result = _upcCode <=> rhs._upcCode;   //operator will output 0 if equal, neg int if lhs>rhs, pos int if lhs<rhs
  if (result != 0)                           //if upcCode compared are not equal, end and return. Otherwise, continue checking
  {
    return result;
  }

  result = _productName <=> rhs._productName;
  if (result != 0)                           //if productName compared are not equal, end and return. Otherwise, continue checking
  {
    return result;
  }

  result = _brandName <=> rhs._brandName;
  if (result != 0)                           //if brandName compared are not equal, end and return. Otherwise, continue checking
  {
    return result;
  }

  //avoid _price <=> rhs._price
  //1. floating point three-way comparison yields a partial ordering, not convertible to a weak_ordering
  //2. Use the function we defined earlier floating_point_is_equal instead
  if (floating_point_is_equal(_price, rhs._price))    //if lhs _price == rhs _price
  {
    return std::weak_ordering::equivalent;        
  }
  if (_price < rhs._price)
  {
    return std::weak_ordering::less;                  //if lhs _price < rhs _price
  }
  else                                                
  {
    return std::weak_ordering::greater;               //if lhs _price > rhs _price
  }
  /////////////////////// END-TO-DO (19) ////////////////////////////
}
...
bool GroceryItem::operator==( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (20) //////////////////////////////
  return (floating_point_is_equal(_price, rhs._price) &&    //more efficient way than comparing variables one by one with if()
          _upcCode     == rhs._upcCode                &&    //this method uses and relies on short-circuit boolean algebra
          _brandName   == rhs._brandName              &&
          _productName == rhs._productName);
  /////////////////////// END-TO-DO (20) ////////////////////////////
}
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ///////////////////////// TO-DO (21) //////////////////////////////
    ...
    // Except for "char" type objects (like the delimiter), the default stream extraction behaviour is to skip
    // leading whitespace (spaces, tabs, newlines, etc) when reading the next item in the stream. But the default
    // can be changed before this function is called. To remove the assumption white space is skipped, the std::ws manipulator
    // is explicitly used to skip leading whitespace.
    ...
    {
      //if all is ok, move workingItem into the returned grocery item. Copying the information works but moving is more efficient.
      //this uses GroceryItem's move assignment operator.
